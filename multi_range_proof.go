...
func CalculateLMRP(aL, sL []*big.Int, z, x *big.Int, EC CryptoParams) []*big.Int {
	result := make([]*big.Int, len(aL))

	tmp1 := VectorAddScalar(aL, new(big.Int).Neg(z), EC)
	tmp2 := ScalarVectorMul(sL, x, EC)

	result = VectorAdd(tmp1, tmp2, EC)

	return result
}

func CalculateRMRP(aR, sR, y, zTimesTwo []*big.Int, z, x *big.Int, EC CryptoParams) []*big.Int {
	...
	tmp11 := VectorAddScalar(aR, z, EC)
	tmp12 := ScalarVectorMul(sR, x, EC)
	tmp1 := VectorHadamard(y, VectorAdd(tmp11, tmp12, EC), EC)

	result = VectorAdd(tmp1, zTimesTwo, EC)
...
func DeltaMRP(y []*big.Int, z *big.Int, m int,EC CryptoParams) *big.Int {
	result := big.NewInt(0)
...
*/
func MRPProve(values []*big.Int,EC CryptoParams) MultiRangeProof {
	...
	for j := range values {
		...
		check(err)
		Comms[j] = EC.G.Mult(v, EC).Add(EC.H.Mult(gamma, EC), EC)
		gammas[j] = gamma
	...
	A := TwoVectorPCommitWithGens(EC.BPG, EC.BPH, aLConcat, aRConcat, EC).Add(EC.H.Mult(alpha, EC), EC)
	MRPResult.A = A

	sL := RandVector(EC.V, EC)
	sR := RandVector(EC.V, EC)
	...
	*/
	...
	l0 := VectorAddScalar(aLConcat, new(big.Int).Neg(cz), EC)
	l1 := sL
	r0 := VectorAdd(
		VectorHadamard(
			PowerOfCY,
			VectorAddScalar(aRConcat, cz, EC), EC),
		zPowersTimesTwoVec, EC)
	r1 := VectorHadamard(sR, PowerOfCY, EC)
	...
	t0 := new(big.Int).Mod(new(big.Int).Add(vz2, DeltaMRP(PowerOfCY, cz, m, EC)), EC.N)
	...
	left := CalculateLMRP(aLConcat, sL, cz, cx, EC)
	right := CalculateRMRP(aRConcat, sR, PowerOfCY, zPowersTimesTwoVec, cz, cx, EC)
	...
	MRPResult.IPP = InnerProductProve(left, right, that, P, EC.U, EC.BPG, HPrime, EC)
...
*/
func MRPVerify(mrp MultiRangeProof,EC CryptoParams) bool {
	m := len(mrp.Comms)
	bitsPerValue := EC.V / m

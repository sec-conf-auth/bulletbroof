...
*/
func VectorPCommit(value []*big.Int, EC CryptoParams) (ECPoint, []*big.Int) {
	R := make([]*big.Int, EC.V)
	...
	for i := 0; i < EC.V; i++ {
		...
		commitment = commitment.Add(ECPoint{lhsX, lhsY}, EC).Add(ECPoint{rhsX, rhsY}, EC)
	}
...
for each element and for each randomness.
*/
func TwoVectorPCommit(a []*big.Int, b []*big.Int, EC CryptoParams) ECPoint {
	if len(a) != len(b) {
	...
	for i := 0; i < EC.V; i++ {
		commitment = commitment.Add(EC.BPG[i].Mult(a[i], EC), EC).Add(EC.BPH[i].Mult(b[i],EC), EC)
	}
...
*/
func TwoVectorPCommitWithGens(G, H []ECPoint, a, b []*big.Int, EC CryptoParams) ECPoint {
	...
	for i := 0; i < len(G); i++ {
		...
		commitment = commitment.Add(G[i].Mult(modA, EC), EC).Add(H[i].Mult(modB, EC), EC)
	}

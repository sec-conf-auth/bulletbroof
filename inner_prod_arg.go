...
func GenerateNewParams(G, H []ECPoint, x *big.Int, L, R, P ECPoint, EC CryptoParams) ([]ECPoint, []ECPoint, ECPoint) {
	nprime := len(G) / 2
	...
	for i := range Gprime {
		...
		Gprime[i] = G[i].Mult(xinv, EC).Add(G[i+nprime].Mult(x, EC), EC)
		Hprime[i] = H[i].Mult(x, EC).Add(H[i+nprime].Mult(xinv, EC), EC)
	}

	x2 := new(big.Int).Mod(new(big.Int).Mul(x, x), EC.N)
	xinv2 := new(big.Int).ModInverse(x2, EC.N)

	Pprime := L.Mult(x2, EC).Add(P, EC).Add(R.Mult(xinv2,EC), EC) // x^2 * L + P + xinv^2 * R
...
*/
func InnerProductProveSub(proof InnerProdArg, G, H []ECPoint, a []*big.Int, b []*big.Int, u ECPoint, P ECPoint,EC CryptoParams) InnerProdArg {
	...
	//fmt.Println(nprime)
	//fmt.Println(len(H))
	cl := InnerProduct(a[:nprime], b[nprime:], EC) // either this line
	cr := InnerProduct(a[nprime:], b[:nprime], EC) // or this line
	L := TwoVectorPCommitWithGens(G[nprime:], H[:nprime], a[:nprime], b[nprime:], EC).Add(u.Mult(cl,EC), EC)
	R := TwoVectorPCommitWithGens(G[:nprime], H[nprime:], a[nprime:], b[:nprime], EC).Add(u.Mult(cr,EC), EC)
	...
	Gprime, Hprime, Pprime := GenerateNewParams(G, H, x, L, R, P, EC)
	//fmt.Printf("Prover - Intermediate Pprime value: %s \n", Pprime)
	xinv := new(big.Int).ModInverse(x, EC.N)

	// or these two lines
	aprime := VectorAdd(
		ScalarVectorMul(a[:nprime], x, EC),
		ScalarVectorMul(a[nprime:], xinv, EC), EC)
	bprime := VectorAdd(
		ScalarVectorMul(b[:nprime], xinv, EC),
		ScalarVectorMul(b[nprime:], x, EC), EC)

	return InnerProductProveSub(proof, Gprime, Hprime, aprime, bprime, u, Pprime,EC)
}

func InnerProductProve(a []*big.Int, b []*big.Int, c *big.Int, P, U ECPoint, G, H []ECPoint, EC CryptoParams) InnerProdArg {
	...
	Pprime := P.Add(U.Mult(new(big.Int).Mul(new(big.Int).SetBytes(x[:]), c), EC), EC)
	ux := U.Mult(new(big.Int).SetBytes(x[:]), EC)
	//fmt.Printf("Prover Pprime value to run sub off of: %s\n", Pprime)
	return InnerProductProveSub(runningProof, G, H, a, b, ux, Pprime, EC)
}
...
*/
func InnerProductVerify(c *big.Int, P, U ECPoint, G, H []ECPoint, ipp InnerProdArg,EC CryptoParams) bool {
	...
	Gprime := G
	Hprime := H
	Pprime := P.Add(ux.Mult(c,EC), EC) // line 6 from protocol 1
	...
	for curIt >= 0 {
		...
		Gprime, Hprime, Pprime = GenerateNewParams(Gprime, Hprime, chal2, Lval, Rval, Pprime, EC)
		curIt -= 1
	}
	ccalc := new(big.Int).Mod(new(big.Int).Mul(ipp.A, ipp.B), EC.N)

	Pcalc1 := Gprime[0].Mult(ipp.A, EC)
	Pcalc2 := Hprime[0].Mult(ipp.B, EC)
	Pcalc3 := ux.Mult(ccalc, EC)
	Pcalc := Pcalc1.Add(Pcalc2, EC).Add(Pcalc3, EC)
...
func InnerProductVerifyFast(c *big.Int, P, U ECPoint, G, H []ECPoint, ipp InnerProdArg,EC CryptoParams) bool {
	//fmt.Println("Verifying Inner Product Argument")
	//fmt.Printf("Commitment Value: %s \n", P)
	s1 := sha256.Sum256([]byte(P.X.String() + P.Y.String()))
	chal1 := new(big.Int).SetBytes(s1[:])
	...
	for j := curIt; j >= 0; j-- {
		...
		//fmt.Println(tmp1)
		tmp1 = ipp.L[j].Mult(x2, EC).Add(ipp.R[j].Mult(x2i,EC), EC).Add(tmp1, EC)
		//fmt.Println(tmp1)
	}
	rhs := Pprime.Add(tmp1, EC)
	...
	ccalc := new(big.Int).Mod(new(big.Int).Mul(ipp.A, ipp.B), EC.N)
	lhs := TwoVectorPCommitWithGens(G, H, ScalarVectorMul(sScalars, ipp.A, EC), ScalarVectorMul(invsScalars, ipp.B, EC), EC).Add(ux.Mult(ccalc, EC), EC)
